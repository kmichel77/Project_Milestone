...
LACounty_1 <- LACounty_flu %>% 
  ...
  select(-race_eth, -dt_report, -date)

CA_flu1 <- CA_flu %>% 
  rename("susceptible" = count_susceptible) %>% 
  rename("new_infected" = new_infections) %>% 
  rename("age_category" = age_cat) %>% 
  select(-time_int, -race_ethnicity, -report_date, -dt_diagnosis, -pop, -new_unrecovered, -cumulative_unrecovered)
...
CA_flu_merge <- merge(x = CA_flu1, y = LACounty_1, by = c("county", "age_category", "sex", "new_infected", "new_recovered", "susceptible", "current_infected", "cumulative_infected", "cumulative_recovered", "new_severe", "cumulative_severe"), all = TRUE)

CA_flu_merge_county <- CA_flu_merge %>% 
  group_by(county) %>% 
